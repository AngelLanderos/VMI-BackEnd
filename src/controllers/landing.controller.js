...
LandingController.createLanding = async (req, res) => {
  try {
    ...
    const { customer, landingType, order, createdBy,partNumbers } = journeyData;
    ...
    // primer stage confirmado
    stages[0].confirmationDate = new Date();
    stages[0].docValue = order;
   
    //TODO Consultar SNP del catalago y calcular el numero de cajas 
...
LandingController.getLandingPartNumbers = async (req, res) => {
  try {
    ...
    const landingPartNumbers = await LandingModel.findOne({ID: landingID},{_id:0, partNumbers: 1});
